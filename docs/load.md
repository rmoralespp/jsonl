...
Deserialize a JSON Lines source into an iterator of Python objects. Supports `str` and `bytes` filenames, URLs,
...
|--------------|----------------------------------------------------|----------------------|-------------------------------------------------------------------------------------|
| `source`     | `str`, `bytes`, `PathLike`, URL, `Request`, file-like | *(required)*    | The JSON Lines source to read from                                                  |
...
!!! note
    ...
    `bytes` paths and `PathLike` objects returning bytes are decoded with `os.fsdecode()`. URLs must be provided as
    `str` or `urllib.request.Request`; a `bytes` value always represents a filesystem path.
